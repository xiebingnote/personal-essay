...
if [ "$execute" == "Y" ]
  ...
  sleep 1
  sh etcd-cert.sh
  cp ca.crt etcd.crt etcd.key $certpath
  echo
...
Group=root
ExecStart=/bin/etcd \\
--cert-file=$certpath/etcd.crt \\
--key-file=$certpath/etcd.key \\
--peer-cert-file=$certpath/etcd.crt \\
--peer-key-file=$certpath/etcd.key \\
--trusted-ca-file=$certpath/ca.crt \\
--peer-trusted-ca-file=$certpath/ca.crt \\
--client-cert-auth \\
