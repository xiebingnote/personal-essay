...
⚠️注意：execution需要和jobmanager和taskmanager同级
    ...
    2. 添加如下配置：timeout和interval单位ms
       execution:
         checkpointing:
           ...
           max-concurrent: 1
           min-pause: 1000
    3. 同时修改numberOfTaskSlots的值，默认为1，只能启动一个job任务，根据实际情况修改
...
#### 1. 依赖包（文件在flink-cdc目录下）
        ...
        flink-cdc-3.3.0（文件夹）
        mysql-connector-java-8.0.27.jar
        flink-cdc-pipeline-connector-starrocks-3.3.0.jar
        flink-cdc-pipeline-connector-mysql-3.3.0.jar
        注: flink-cdc-3.3.0文件夹放到job容器内的/opt/flink/lib目录下
            下载的flink-cdc-3.3.0-bin.tar.gz文件解压后，lib目录下无flink-cdc-pipeline-connector-starrocks-3.3.0.jar和flink-cdc-pipeline-connector-mysql-3.3.0.jar包,需自行加入
...
##### 2. 配置yml文件
    ...
    文件名mysql-to-starrocks.yaml（可自行修改文件名），文件上传到jobmanager容器内的flink-cdc-3.3.0目录下，文件配置内容如下，根据实际情况进行修改：
